...
from myapp.models import Advantage
from myapp.permission.permission import isDemoAdminUser, check_if_demo
from myapp.serializers import AdvantageSerializer
...
def list_api(request):
    if request.method == 'GET':
        ...
        paginated_advantage = paginator.paginate_queryset(advantage, request)
        serializer = AdvantageSerializer(paginated_advantage, many=True)
...
def update(request):
    ...
    data = request.data.copy()
    serializer = AdvantageSerializer(advantage, data=data)
    if serializer.is_valid():
        serializer.save()
...
def delete(request):
    ...
    try:
        ...
        ids_arr = ids.split(',')
        Advantage.objects.filter(id__in=ids_arr).delete()
    except Advantage.DoesNotExist:
        return APIResponse(code=1, msg='对象不存在')
